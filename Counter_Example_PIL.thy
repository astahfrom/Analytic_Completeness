...
definition my_gframe :: \<open>int gframe\<close> where
  \<open>my_gframe \<equiv> \<lparr> \<W> = UNIV, \<R> = \<lambda>x. {}, \<Pi> = Pss \<rparr>\<close>
...
abbreviation \<open>nowhere_or_twice p \<equiv>
  \<^bold>\<down> (\<^bold>E (\<^bold>\<down>(\<^bold>@ (\<^bold>#0) p))) \<^bold>\<longrightarrow>
  \<^bold>E (\<^bold>\<down> (\<^bold>E (\<^bold>\<down> (
