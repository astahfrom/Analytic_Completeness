...
text \<open>
  This is Anders Schlichtkrull's Q0_Soundness.Q0 theory with the following modifications:
  - The syntax is parametric over the choice of constant symbols.
    - This gives us \<open>map_trm\<close> and functoriality lemmas for free.
  - The logical constants Q and \<iota> have designated constructors to distinguish them
      syntactically from the free constants (parameters).
  - \<open>y\<^sub>i\<close> has been made a variable to make axiom 5 parametric in the choice of constant symbols.

  One question:
  - Do we want both the \<open>Cst\<close> and \<open>Par\<close> constructors to carry the type?

  Source: \<^url>\<open>https://www.isa-afp.org/entries/Q0_Soundness.html\<close>
\<close>

theory Q0_2
  imports "Q0_Soundness.Set_Theory"
...
datatype (cst_syms: 'cst_sym) cst
  = Q type_sym
  | Iota
  | Par 'cst_sym type_sym

datatype (trm_csts: 'cst_sym) trm =
  Var var_sym type_sym |
  Cst \<open>'cst_sym cst\<close> type_sym  |
  App \<open>'cst_sym trm\<close> \<open>'cst_sym trm\<close> (infixl \<open>\<^bold>\<cdot>\<close> 80) |
  Abs var_sym type_sym \<open>'cst_sym trm\<close> (\<open>\<^bold>[\<^bold>\<lambda>_:_. _\<^bold>]\<close> [80,80,80])

abbreviation Cst_Q (\<open>\<^bold>Q\<close>) where
  \<open>\<^bold>Q \<alpha> \<equiv> Cst (Q \<alpha>) ((Tv \<^bold>\<Leftarrow> \<alpha>) \<^bold>\<Leftarrow> \<alpha>)\<close>

abbreviation Cst_Iota (\<open>\<^bold>\<iota>\<close>) where
  \<open>\<^bold>\<iota> \<equiv> Cst Iota (Ind \<^bold>\<Leftarrow> (Tv \<^bold>\<Leftarrow> Ind))\<close>

fun vars :: "'cst_sym trm \<Rightarrow> (var_sym * type_sym) set" where
  "vars (Var x \<alpha>) = {(x,\<alpha>)}"
| "vars (Cst _ _) = {}"
| "vars (A \<^bold>\<cdot> B) = vars A \<union> vars B"
| "vars (\<^bold>[\<^bold>\<lambda>x:\<alpha>. A\<^bold>]) = {(x,\<alpha>)} \<union> vars A"

fun frees :: "'cst_sym trm \<Rightarrow> (var_sym * type_sym) set" where
...
inductive wff_cst :: "type_sym \<Rightarrow> 'cst_sym cst \<Rightarrow> bool" where
  wff_Q: "wff_cst ((Tv \<^bold>\<Leftarrow> \<alpha>) \<^bold>\<Leftarrow> \<alpha>) (Q \<alpha>)"
| wff_Iota: "wff_cst (Ind \<^bold>\<Leftarrow> (Tv \<^bold>\<Leftarrow> Ind)) Iota"
| wff_Par: "wff_cst \<alpha> (Par c \<alpha>)"

inductive wff :: "type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> bool" where
  wff_Var: "wff \<alpha> (Var _ \<alpha>)"
| wff_Cst: "wff_cst \<alpha> cst \<Longrightarrow> wff \<alpha> (Cst cst \<alpha>)"
...
fun type_of_cst :: "'cst_sym cst \<Rightarrow> type_sym" where
  "type_of_cst (Q \<alpha>) = (Tv \<^bold>\<Leftarrow> \<alpha>) \<^bold>\<Leftarrow> \<alpha>"
| "type_of_cst Iota = Ind \<^bold>\<Leftarrow> (Tv \<^bold>\<Leftarrow> Ind)"
| "type_of_cst (Par c \<alpha>) = \<alpha>"

fun type_of :: "'cst_sym trm \<Rightarrow> type_sym" where
  "type_of (Var x \<alpha>) = \<alpha>"
| "type_of (Cst cst \<alpha>) = \<alpha>"
...
lemma wff_type_of_cst [simp]:
  "wff_cst \<alpha> cst \<Longrightarrow> type_of_cst cst = \<alpha>"
  by (induction cst rule: wff_cst.induct) auto
...
lemma wff_Q'[simp, code]:
  "wff_cst \<beta> (Q \<alpha>) \<longleftrightarrow> \<beta> = (Tv \<^bold>\<Leftarrow> \<alpha>) \<^bold>\<Leftarrow> \<alpha>"
  using wff_Q by (auto elim: wff_cst.cases)

lemma wff_Iota'[simp, code]:
  "wff_cst \<beta> Iota \<longleftrightarrow> \<beta> = Ind \<^bold>\<Leftarrow> (Tv \<^bold>\<Leftarrow> Ind)"
  using wff_Iota by (auto elim: wff_cst.cases)

lemma wff_Par'[simp, code]:
  "wff_cst \<beta> (Par c \<alpha>) \<longleftrightarrow> \<beta> = \<alpha>"
  using wff_Par by (auto elim: wff_cst.cases)

lemma wff_Cst'[simp, code]:
  "wff \<beta> (Cst cst \<alpha>) \<longleftrightarrow> \<alpha> = \<beta> \<and> wff_cst \<beta> cst"
  using wff_Cst by (auto elim: wff.cases)
...
lemma unique_type_cst:
  "wff_cst \<beta> cst \<Longrightarrow> wff_cst \<alpha> cst \<Longrightarrow> \<alpha> = \<beta>"
  by (induction cst arbitrary: \<alpha> rule: wff_cst.induct) simp_all
...
next
  ...
  then show ?case
    using wff_Abs_type_of by metis
qed


section \<open>Replacement\<close>

inductive replacement :: "'cst_sym trm \<Rightarrow> 'cst_sym trm \<Rightarrow> 'cst_sym trm \<Rightarrow> 'cst_sym trm \<Rightarrow> bool" where
...
next
  ...
  have \<beta>: "\<beta> = \<beta>' \<^bold>\<Leftarrow> \<alpha>'"
    using wff_D unique_type replace_Abs by (metis wff_Abs_type_of)
...
definition Eql :: "'cst_sym trm \<Rightarrow> 'cst_sym trm \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm" where
  "Eql A B \<alpha> \<equiv> (\<^bold>Q \<alpha>) \<^bold>\<cdot> A \<^bold>\<cdot> B"

abbreviation Eql' :: "'cst_sym trm \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> 'cst_sym trm" (\<open>\<^bold>[_ \<^bold>=_\<^bold>= _\<^bold>]\<close> [89]) where
...
definition T :: \<open>'cst_sym trm\<close> where
  "T \<equiv> \<^bold>[\<^bold>Q Tv \<^bold>=ooo\<^bold>= \<^bold>Q Tv\<^bold>]"
...
abbreviation F :: \<open>'cst_sym trm\<close> where
...
definition PI :: "type_sym \<Rightarrow> 'cst_sym trm" where
  "PI \<alpha> \<equiv> \<^bold>Q (Tv \<^bold>\<Leftarrow> \<alpha>) \<^bold>\<cdot> \<^bold>[\<^bold>\<lambda> ''x'':\<alpha>. T\<^bold>]"
...
definition Forall :: "var_sym \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> 'cst_sym trm"
  (\<open>\<^bold>[\<^bold>\<forall>_:_. _\<^bold>]\<close> [80,80,80]) where 
...
definition Con_sym :: \<open>'cst_sym trm\<close> where
  "Con_sym \<equiv>
...
definition Con :: "'cst_sym trm \<Rightarrow> 'cst_sym trm \<Rightarrow> 'cst_sym trm" (infix \<open>\<^bold>\<and>\<close> 80) where
...
definition Imp_sym :: \<open>'cst_sym trm\<close> where
...
definition Imp :: "'cst_sym trm \<Rightarrow> 'cst_sym trm \<Rightarrow> 'cst_sym trm" (infix \<open>\<^bold>\<longrightarrow>\<close> 80) where
...
definition axiom_1 :: \<open>'cst_sym trm\<close> where
...
definition axiom_2 :: "type_sym \<Rightarrow> 'cst_sym trm" where
  "axiom_2 \<alpha> \<equiv>
...
definition axiom_3 :: "type_sym \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm" where
...
definition axiom_4_1 :: "var_sym \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> 'cst_sym trm" where
  "axiom_4_1 x \<alpha> B \<beta> A \<equiv> \<^bold>[(\<^bold>[\<^bold>\<lambda>x:\<alpha>. B\<^bold>] \<^bold>\<cdot> A) \<^bold>=\<beta>\<^bold>= B\<^bold>]"

definition axiom_4_1_side_condition :: "var_sym \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> bool" where
  "axiom_4_1_side_condition x \<alpha> B \<beta> A \<equiv> (\<exists>c. B = Cst (Par c \<beta>) \<beta>) \<or> (\<exists>y. B = Var y \<beta> \<and> (x \<noteq> y \<or> \<alpha> \<noteq> \<beta>))"

definition axiom_4_2 :: "var_sym \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> 'cst_sym trm" where
  "axiom_4_2 x \<alpha> A = \<^bold>[(\<^bold>[\<^bold>\<lambda>x:\<alpha>. Var x \<alpha>\<^bold>] \<^bold>\<cdot> A) \<^bold>=\<alpha>\<^bold>= A\<^bold>]"

definition axiom_4_3 :: "var_sym \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow>
                          type_sym \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> 'cst_sym trm  \<Rightarrow> 'cst_sym trm" where
...
definition axiom_4_4 :: "var_sym \<Rightarrow> type_sym \<Rightarrow> var_sym \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> 'cst_sym trm" where
...
definition axiom_4_4_side_condition :: "var_sym \<Rightarrow> type_sym \<Rightarrow> var_sym \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> bool" where
  "axiom_4_4_side_condition x \<alpha> y \<gamma> B \<delta> A \<equiv> (x \<noteq> y \<or> \<alpha> \<noteq> \<gamma>) \<and> (y, \<gamma>) \<notin> vars A"

definition axiom_4_5 :: "var_sym \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> 'cst_sym trm" where
...
definition axiom_5 :: \<open>'cst_sym trm\<close> where
  "axiom_5 = \<^bold>[(\<^bold>\<iota> \<^bold>\<cdot> (\<^bold>Q Ind \<^bold>\<cdot> y\<^sub>i)) \<^bold>=Ind\<^bold>= y\<^sub>i\<^bold>]"

inductive axiom :: "'cst_sym trm \<Rightarrow> bool" where
  by_axiom_1: 
  "axiom axiom_1"
| by_axiom_2: 
  "axiom (axiom_2 \<alpha>)"
...
inductive rule_R :: "'cst_sym trm \<Rightarrow> 'cst_sym trm \<Rightarrow> 'cst_sym trm \<Rightarrow> bool" where
  "replacement A B C D \<Longrightarrow> rule_R C (\<^bold>[A \<^bold>=\<alpha>\<^bold>= B\<^bold>]) D"

definition "proof" :: "'cst_sym trm \<Rightarrow> 'cst_sym trm list \<Rightarrow> bool" where
...
inductive "theorem" :: "'cst_sym trm \<Rightarrow> bool" where
...
(* NOTE: I have changed the second var_sym to 'cst_sym here *)
definition axiom_4_1_variant_cst :: "var_sym \<Rightarrow> type_sym \<Rightarrow> 'cst_sym \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> 'cst_sym trm" where
  "axiom_4_1_variant_cst x \<alpha> c \<beta> A \<equiv> \<^bold>[(\<^bold>[\<^bold>\<lambda>x:\<alpha>. Cst (Par c \<beta>) \<beta>\<^bold>] \<^bold>\<cdot> A) \<^bold>=\<beta>\<^bold>= (Cst (Par c \<beta>) \<beta>)\<^bold>]"

definition axiom_4_1_variant_var :: "var_sym \<Rightarrow> type_sym \<Rightarrow> var_sym \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> 'cst_sym trm" where
...
definition axiom_4_1_variant_var_side_condition :: "var_sym \<Rightarrow> type_sym \<Rightarrow> var_sym \<Rightarrow> type_sym \<Rightarrow> 'cst_sym trm \<Rightarrow> bool" where
...
type_synonym ('cst_sym, 's) denotation = "'cst_sym cst \<Rightarrow> type_sym \<Rightarrow> 's"
...
inductive wf_interp :: "'s frame \<Rightarrow> ('cst_sym, 's) denotation \<Rightarrow> bool" where
  "wf_frame D \<Longrightarrow>
   \<forall>c \<alpha>. I (Par c \<alpha>) \<alpha> \<in>: D \<alpha> \<Longrightarrow>
   \<forall>\<alpha>. I (Q \<alpha>) (Tv \<^bold>\<Leftarrow> \<alpha> \<^bold>\<Leftarrow> \<alpha>) = iden (D \<alpha>) \<Longrightarrow>
   (I Iota (Ind \<^bold>\<Leftarrow> (Tv \<^bold>\<Leftarrow> Ind))) \<in>: funspace (D (Tv \<^bold>\<Leftarrow> Ind)) (D Ind) \<Longrightarrow>
   \<forall>x. x \<in>: D Ind \<longrightarrow> (I Iota (Ind \<^bold>\<Leftarrow> (Tv \<^bold>\<Leftarrow> Ind))) \<cdot> one_elem_fun x (D Ind) = x \<Longrightarrow>
   wf_interp D I"
...
abbreviation (input) asg_into_interp :: "'s asg \<Rightarrow> 's frame \<Rightarrow> ('cst_sym, 's) denotation \<Rightarrow> bool" where
...
primrec val :: "'s frame \<Rightarrow> ('cst_sym, 's) denotation \<Rightarrow> 's asg \<Rightarrow> 'cst_sym trm \<Rightarrow> 's" where
  "val D _ \<phi> (Var x \<alpha>) = \<phi> (x,\<alpha>)"
...
definition general_model :: "'s frame \<Rightarrow> ('cst_sym, 's) denotation \<Rightarrow> bool" where
  "general_model D I \<longleftrightarrow> wf_interp D I \<and>
    (\<forall>\<phi> A \<alpha>. asg_into_interp \<phi> D I \<longrightarrow> wff \<alpha> A \<longrightarrow> val D I \<phi> A \<in>: D \<alpha>)"

definition standard_model :: "'s frame \<Rightarrow> ('cst_sym, 's) denotation \<Rightarrow> bool" where
...
lemma asg_into_interp_fun_upd:
  ...
  using assms asg_into_frame_fun_upd unfolding general_model_def by fastforce
...
proof -
  have "wf_interp D I"
    using assms unfolding standard_model_def by auto
  moreover
  have "wff \<alpha> A \<Longrightarrow> asg_into_interp \<phi> D I \<Longrightarrow> val D I \<phi> A \<in>: D \<alpha>"
    for \<phi> \<alpha> A
  proof (induction arbitrary: \<phi> rule: wff.induct)
    ...
    then show ?case 
      using assms boolean_in_boolset unfolding standard_model_def
      by (auto simp: iden_def one_elem_fun_def wf_frame_def elim!: wf_interp.cases wff_cst.cases)
  next
    case (wff_App \<alpha> \<beta> A B)
    then show ?case
      using apply_in_rng assms unfolding standard_model_def
      by fastforce
  next
    case (wff_Abs \<beta> A \<alpha> x)
    then show ?case 
      using assms unfolding standard_model_def
      by (fastforce simp: asg_into_frame_fun_upd)
  qed
  ultimately
  show "general_model D I"
    unfolding general_model_def by auto
qed
...
next
  ...
  proof (rule abstract_extensional, rule, rule)
    ...
    then have "val D I (\<phi>((x, \<alpha>) := xa)) A \<in>: D \<beta>"
      using wff_Abs asg_into_frame_fun_upd unfolding general_model_def
      by meson
    moreover
    {
...
abbreviation satisfies :: "'s frame \<Rightarrow> ('cst_sym, 's) denotation \<Rightarrow> 's asg \<Rightarrow> 'cst_sym trm \<Rightarrow> bool" where
  "satisfies D I \<phi> A \<equiv> (val D I \<phi> A = true)"

definition valid_in_model :: "'s frame \<Rightarrow> ('cst_sym, 's) denotation \<Rightarrow> 'cst_sym trm \<Rightarrow> bool" where
  "valid_in_model D I A \<equiv> (\<forall>\<phi>. asg_into_interp \<phi> D I \<longrightarrow> val D I \<phi> A = true)"

definition valid_general :: "'cst_sym trm \<Rightarrow> bool" where
  "valid_general A \<equiv> \<forall>D I. general_model D I \<longrightarrow> valid_in_model D I A"

definition valid_standard :: "'cst_sym trm \<Rightarrow> bool" where
...
proof -
  have val_A: "val D I \<phi> A \<in>: D \<alpha>"
    using assms unfolding general_model_def by simp
  have "asg_into_interp (\<phi>((x, \<alpha>) := val D I \<phi> A)) D I"
    using assms unfolding general_model_def by (simp add: asg_into_frame_fun_upd)
...
proof -
  have "val D I \<phi> (\<^bold>[A \<^bold>=\<alpha>\<^bold>= B\<^bold>]) = (iden (D \<alpha>)) \<cdot> val D I \<phi> A \<cdot> val D I \<phi> B"
    using assms(1) unfolding Eql_def general_model_def by (auto elim: wf_interp.cases)
  also have "... = boolean (val D I \<phi> A = val D I \<phi> B)"
    using apply_id using assms unfolding general_model_def by blast
  finally show ?thesis 
    unfolding Eql_def by simp
qed
...
lemma lemma_5401_c:
  ...
  using assms lemma_5401_b[OF assms(1,2)] unfolding T_def general_model_def by auto
...
proof -
  have "iden boolset \<in>: D ooo"
    using assms unfolding general_model_def wf_interp.simps wf_frame_def 
    by (metis wff_Cst wff_Q  val.simps(2))
  then have "(val D I \<phi> \<^bold>[\<^bold>\<lambda>''x'':Tv. T\<^bold>]) \<cdot> false \<noteq> (val D I \<phi> \<^bold>[\<^bold>\<lambda>''x'':Tv. x\<^sub>o\<^bold>]) \<cdot> false" 
    using assms true_neq_false
      apply_id[of "iden boolset" "(D ooo)" "iden boolset"]
    unfolding boolean_def Eql_def T_def general_model_def
    by (auto simp: wf_frame_def elim: wf_interp.cases)
  ...
  have "val D I \<phi> F = boolean (val D I \<phi> (\<^bold>[\<^bold>\<lambda>''x'':Tv. T\<^bold>]) = val D I \<phi> \<^bold>[\<^bold>\<lambda>''x'':Tv. x\<^sub>o\<^bold>])"
    using assms by (auto simp: lemma_5401_b_variant_1 )
  also
  have "... = boolean False"
    using neqLR by auto
  also
    ...
    proof (rule abstract_extensional')
      ...
      then have "val D I (\<phi>''' x) (g\<^sub>o\<^sub>o\<^sub>o \<^bold>\<cdot> T \<^bold>\<cdot> T) \<in>: D Tv"
        using \<phi>'''_def \<phi>''_asg_into asg_into_frame_fun_upd assms(1) 
          general_model_def type_sym.inject wff_T 
    ...
    next
      ...
      then have "val D I (\<phi>''' x) T = true"
        unfolding \<phi>'''_def using  \<phi>''_asg_into asg_into_frame_fun_upd 
          lemma_5401_c[OF assms(1)] by fast
      then show "val D I (\<phi>''' x) g\<^sub>o\<^sub>o\<^sub>o \<cdot> val D I (\<phi>''' x) T \<cdot>
                   val D I (\<phi>''' x) T =
...
lemma lemma_5401_e_2:
  fixes I :: \<open>('cst_sym, 's) denotation\<close>
  assumes "general_model D I"
  assumes "asg_into_interp \<phi> D I"
  assumes "y = true \<or> y = false"
  shows "(val D I \<phi> Con_sym) \<cdot> false \<cdot> y = false"
proof -
  define give_x :: \<open>'cst_sym trm\<close> where "give_x = \<^bold>[\<^bold>\<lambda>''y'':Tv. x\<^sub>o\<^bold>]"
  define give_fst :: \<open>'cst_sym trm\<close> where "give_fst = \<^bold>[\<^bold>\<lambda> ''x'':Tv. give_x\<^bold>]"
  ...
  have "\<And>a b. a \<in>: D Tv \<Longrightarrow> 
    ...
    using wff_give_x asg_into_frame_def assms(1,2) unfolding general_model_def by (auto simp: asg_into_frame_fun_upd)
  moreover
  ...
  then have val_give_fst_fun: "val_give_fst \<in>: D ooo"
    unfolding val_give_fst_def using assms unfolding general_model_def by auto

  have "val D I (\<phi>((''x'', Tv) := false, 
                   (''y'', Tv) := y, 
                   (''g'', ooo) := val_give_fst)) T \<in>: D Tv"
    using assms unfolding general_model_def
    by (metis asg_into_frame_fun_upd asg_into_interp_fun_upd_false assms(1) lemma_5401_c
        val_give_fst_fun wff_T)
  ...
  have false_y_TV: "false \<in>: D Tv \<and> y \<in>: D Tv"
    using assms(1) assms(3) unfolding general_model_def
    by (auto simp: wf_frame_def elim: wf_interp.cases)
  then have val_give_fst_in_D: "val_give_fst \<cdot> false \<cdot> y \<in>: D Tv"
    using val_give_fst_simp by auto

  have "true \<in>: D Tv"
    by (metis assms(1) assms(2) general_model_def lemma_5401_c[OF assms(1,2)] wff_T)
  ...
  then have val_give_fst_not_false: 
    ...
    using asg_into_frame_fun_upd assms(1) assms(2) lemma_5401_c false_y_TV val_give_fst_fun by metis
  ...
  have "asg_into_frame (\<phi>((''x'', Tv) := false, (''y'', Tv) := y)) D"
    using asg_into_interp_fun_upd_false[OF assms(1)] general_model_def assms wff_Con_sym_subterm1
  ...
  have "y \<in>: D Tv"
    using general_model_def lemma_5401_d[OF assms(1,2)] wff_F assms
  ...
  have "val D I (\<phi>((''x'', Tv) := false, (''y'', Tv) := y)) \<^bold>[\<^bold>[\<^bold>\<lambda>''g'':ooo. g\<^sub>o\<^sub>o\<^sub>o \<^bold>\<cdot> T \<^bold>\<cdot> T\<^bold>] \<^bold>=(Tv \<^bold>\<Leftarrow> ooo)\<^bold>= \<^bold>[\<^bold>\<lambda>''g'':ooo. g\<^sub>o\<^sub>o\<^sub>o \<^bold>\<cdot> x\<^sub>o \<^bold>\<cdot> y\<^sub>o\<^bold>]\<^bold>] \<in>: D Tv"
    using asg_into_interp_fun_upd_false[OF assms(1)] general_model_def assms wff_Con_sym_subterm1 
      asg_into_interp_fun_upd_true[OF assms(1)] by metis
  moreover
  ...
  have 11: "val D I (\<phi>((''x'', Tv) := false)) \<^bold>[\<^bold>\<lambda> ''y'':Tv. \<^bold>[\<^bold>[\<^bold>\<lambda>''g'':ooo. g\<^sub>o\<^sub>o\<^sub>o \<^bold>\<cdot> T \<^bold>\<cdot> T\<^bold>] \<^bold>=(Tv \<^bold>\<Leftarrow> ooo)\<^bold>= \<^bold>[\<^bold>\<lambda>''g'':ooo. g\<^sub>o\<^sub>o\<^sub>o \<^bold>\<cdot> x\<^sub>o \<^bold>\<cdot> y\<^sub>o\<^bold>]\<^bold>]\<^bold>] \<in>: D oo"
    using asg_into_interp_fun_upd_false[OF assms(1,2)] general_model_def assms
...
proof -
  ...
  define give_y :: \<open>'cst_sym trm\<close> where "give_y = (\<^bold>[\<^bold>\<lambda> ''y'':Tv. y\<^sub>o\<^bold>])"
  define give_snd :: \<open>'cst_sym trm\<close> where "give_snd = \<^bold>[\<^bold>\<lambda> ''x'':Tv. give_y\<^bold>]"
  ...
  have "\<And>a b. a \<in>: D Tv \<Longrightarrow> b \<in>: D Tv \<Longrightarrow> val D I (\<phi>((''x'', Tv) := a)) give_y \<in>: D (type_of give_y)"
    using wff_give_y asg_into_frame_def assms(1) assms(2) unfolding general_model_def by (auto simp: asg_into_frame_fun_upd)
  moreover
  ...
  then have val_give_snd_in_D: "val_give_snd \<in>: D ooo"
    unfolding val_give_snd_def using assms unfolding general_model_def
    by auto
  ...
  then have "val D I (\<phi>((''x'', Tv) := x, 
    ...
    using assms unfolding general_model_def
    by (meson asg_into_frame_fun_upd asg_into_interp_fun_upd_false
        asg_into_interp_fun_upd_true assms(1) wff_T)
  ...
  have false_and_x_in_D: "false \<in>: D Tv \<and> x \<in>: D Tv"
    using assms(1,3) wf_frame_def unfolding general_model_def by (auto elim: wf_interp.cases)
  ...
  then have "val D I (\<phi>((''x'', Tv) := x, (''y'', Tv) := false)) \<^bold>[\<^bold>[\<^bold>\<lambda>''g'':ooo. g\<^sub>o\<^sub>o\<^sub>o \<^bold>\<cdot> T \<^bold>\<cdot> T\<^bold>] \<^bold>=(Tv \<^bold>\<Leftarrow> ooo)\<^bold>= \<^bold>[\<^bold>\<lambda>''g'':ooo. g\<^sub>o\<^sub>o\<^sub>o \<^bold>\<cdot> x\<^sub>o \<^bold>\<cdot> y\<^sub>o\<^bold>]\<^bold>] = false"
    using assms(1,2) lemma_5401_b_variant_3 false_and_x_in_D
    by (metis asg_into_frame_fun_upd wff_Con_sym_subterm0_iff wff_T wff_Var)
  ...
  have "val D I (\<phi>((''x'', Tv) := x)) \<^bold>[\<^bold>\<lambda> ''y'':Tv. \<^bold>[\<^bold>[\<^bold>\<lambda>''g'':ooo. g\<^sub>o\<^sub>o\<^sub>o \<^bold>\<cdot> T \<^bold>\<cdot> T\<^bold>] \<^bold>=(Tv \<^bold>\<Leftarrow> ooo)\<^bold>= \<^bold>[\<^bold>\<lambda>''g'':ooo. g\<^sub>o\<^sub>o\<^sub>o \<^bold>\<cdot> x\<^sub>o \<^bold>\<cdot> y\<^sub>o\<^bold>]\<^bold>]\<^bold>] \<in>: D oo"
    by (metis assms(1,3) general_model_def lemma_5401_c[OF assms(1,2)] 
...
proof -
  ...
  then have "val D I \<phi> (Var x Tv) \<in>: D Tv"
    using assms unfolding general_model_def by blast
  then have "val D I \<phi> (Var x Tv) \<in>: boolset"
    using assms unfolding general_model_def wf_interp.simps wf_frame_def by auto
  then show ?thesis
...
proof -
  have "val D I \<phi> A \<in>: D Tv"
    using assms unfolding general_model_def by auto
  then have "val D I \<phi> A \<in>: boolset"
    using assms unfolding general_model_def wf_interp.simps wf_frame_def by force
  then show ?thesis
...
lemma lemma_5401_e_variant_2:
  ...
  using assms wff_Tv_is_true_or_false[of \<phi> D I] unfolding Con_def 
  by (auto simp: lemma_5401_e_variant_1)
...
proof -
  define Imp_subterm2 :: \<open>'cst_sym trm\<close> where
  ...
  have val_Imp_subterm0_false: "val D I (\<phi>((''x'', Tv) := false, (''y'', Tv) := y)) (x\<^sub>o \<^bold>\<and> y\<^sub>o) = false"
    using assms asg_into_interp_fun_upd_false[OF assms(1)] asg_into_interp_fun_upd_true[OF assms(1)] 
      boolean_def by (auto simp: lemma_5401_e_variant_2)
  ...
  then have val_Imp_subterm2_true: "(val D I (\<phi>((''x'', Tv) := false)) \<^bold>[\<^bold>\<lambda> ''y'':Tv. \<^bold>[x\<^sub>o \<^bold>=Tv\<^bold>= (x\<^sub>o \<^bold>\<and> y\<^sub>o)\<^bold>]\<^bold>]) \<cdot> y = true"
    using assms(1,3) unfolding general_model_def by (auto simp: wf_frame_def elim: wf_interp.cases)
  ...
  proof -
    ...
    then have "val D I (\<phi>((''x'', Tv) := false)) \<^bold>[\<^bold>\<lambda> ''y'':Tv. \<^bold>[x\<^sub>o \<^bold>=Tv\<^bold>= (x\<^sub>o \<^bold>\<and> y\<^sub>o)\<^bold>]\<^bold>] = val D I \<phi> \<^bold>[\<^bold>\<lambda>''x'':Tv. \<^bold>[\<^bold>\<lambda> ''y'':Tv. \<^bold>[x\<^sub>o \<^bold>=Tv\<^bold>= (x\<^sub>o \<^bold>\<and> y\<^sub>o)\<^bold>]\<^bold>]\<^bold>] \<cdot> false"
      by (metis (no_types, lifting) assms(1,2) lemma_5401_a lemma_5401_d val.simps(3) wff_F wff_Imp_sym_subterm2)
    then show ?thesis
...
proof -
  ...
  then have "val D I (\<phi>((''x'', Tv) := x, (''y'', Tv) := true)) (x\<^sub>o \<^bold>\<and> y\<^sub>o) = x"
    using assms by (auto simp: lemma_5401_e_variant_2 boolean_def)
  ...
  have val_Imp_subterm2_true: "val D I (\<phi>((''x'', Tv) := x)) \<^bold>[\<^bold>\<lambda> ''y'':Tv. \<^bold>[x\<^sub>o \<^bold>=Tv\<^bold>= (x\<^sub>o \<^bold>\<and> y\<^sub>o)\<^bold>]\<^bold>] \<cdot> true = true"
    using val_Imp_subterm1_true assms(1) unfolding general_model_def by (auto simp: wf_frame_def elim: wf_interp.cases)
  ...
  have "val D I (\<phi>((''x'', Tv) := x)) \<^bold>[\<^bold>\<lambda> ''y'':Tv. \<^bold>[x\<^sub>o \<^bold>=Tv\<^bold>= (x\<^sub>o \<^bold>\<and> y\<^sub>o)\<^bold>]\<^bold>] \<in>: D oo"
    ...
    by (metis assms(1,2,3) general_model_def lemma_5401_c[OF assms(1,2)]
...
proof -
  ...
  have 3: "val D I (\<phi>((''x'', Tv) := true, (''y'', Tv) := false)) (x\<^sub>o \<^bold>\<and> y\<^sub>o) = false"
    using assms by (auto simp: lemma_5401_e_variant_2)
  ...
  have false_Tv: "false \<in>: D Tv"
    using assms(1) unfolding general_model_def by (auto simp: wf_frame_def elim: wf_interp.cases)
  moreover
  ...
  proof -
    ...
    have "val D I (\<phi>((''x'', Tv) := true)) \<^bold>[\<^bold>\<lambda> ''y'':Tv. \<^bold>[x\<^sub>o \<^bold>=Tv\<^bold>= (x\<^sub>o \<^bold>\<and> y\<^sub>o)\<^bold>]\<^bold>] \<in>: D oo"
      using wff_Imp_sym_subterm2 
      by (metis assms(1) general_model_def asg_into_interp_fun_upd_true[OF assms(1,2)])
    moreover
...
proof -
  ...
  then have PI_subterm_in_D: "val D I \<phi> \<^bold>[\<^bold>\<lambda> ''x'':\<alpha>. T\<^bold>] \<in>: D (Tv \<^bold>\<Leftarrow> \<alpha>)"
    using assms(1,2) unfolding general_model_def by blast

  have "wff (Tv \<^bold>\<Leftarrow> \<alpha>) (\<^bold>[\<^bold>\<lambda>x:\<alpha>. A\<^bold>])"
    using assms by auto
  moreover
  have "\<forall>\<phi> :: 's asg. asg_into_frame \<phi> D \<longrightarrow> (\<forall>A \<alpha>. wff \<alpha> A \<longrightarrow> val D I \<phi> A \<in>: D \<alpha>)"
    using assms(1) unfolding general_model_def by auto
  then have "\<forall>t cs. val D I \<phi> \<^bold>[\<^bold>\<lambda>cs:t. A\<^bold>] \<in>: D (Tv \<^bold>\<Leftarrow> t)"
    using wff_Abs assms(1,2,3) unfolding general_model_def by fast
  ...
  have true_and_A_in_D: "\<forall>z. z \<in>: D \<alpha> \<longrightarrow> true \<in>: D Tv \<and> val D I (\<phi>((x, \<alpha>) := z)) A \<in>: D Tv"
    by (metis assms(1,2,3) general_model_def lemma_5401_c[OF assms(1,2)] asg_into_frame_fun_upd wff_T)
  ...
  moreover have "... \<longleftrightarrow> iden (D (Tv \<^bold>\<Leftarrow> \<alpha>))
                            \<cdot> val D I \<phi> \<^bold>[\<^bold>\<lambda> ''x'':\<alpha>. T\<^bold>] \<cdot> val D I \<phi> \<^bold>[\<^bold>\<lambda>x:\<alpha>. A\<^bold>] =
                         true"
    using assms unfolding PI_def general_model_def wf_interp.simps by simp
  ...
  proof -
    have "\<forall>x. x \<in>: D \<alpha> \<longrightarrow> val D I (\<phi>((''x'', \<alpha>) := x)) T \<in>: D Tv \<and> true \<in>: D Tv"
      using true_and_A_in_D assms(1,2) asg_into_frame_fun_upd unfolding general_model_def by (auto simp: asg_into_frame_fun_upd)
    moreover
    have "\<forall>x. x \<in>: D \<alpha> \<longrightarrow> val D I (\<phi>((''x'', \<alpha>) := x)) T \<in>: D Tv \<and> satisfies D I (\<phi>((''x'', \<alpha>) := x)) T"
      using true_and_A_in_D assms(1) assms(2) lemma_5401_c[OF assms(1)] asg_into_frame_fun_upd by (auto simp: asg_into_frame_fun_upd)
    ultimately
  ...
  moreover have "... \<longleftrightarrow> (\<forall>z. z \<in>: D \<alpha> \<longrightarrow> true = val D I (\<phi>((x, \<alpha>) := z)) A)"
    by (metis assms(1,2) general_model_def lemma_5401_c[OF assms(1,2)] wff_T)
  ...
  proof -
    ...
    next
      ...
      then show "\<forall>z. z \<in>: D \<alpha> \<longrightarrow> satisfies D I (\<phi>((x, \<alpha>) := z)) A"
        ...
        unfolding agree_off_asg_def by (auto simp: asg_into_frame_fun_upd)
    qed
  qed
...
proof -
  ...
  then show ?thesis
    using assms by (auto simp: lemma_5401_g boolean_def)
qed
...
(* TODO: this feels silly with the duplicated types *)
lemma fun_sym_interp_to_funspace:
  assumes "asg_into_frame \<phi> D"
  assumes "general_model D I"
  shows "I (Par f (\<alpha> \<^bold>\<Leftarrow> \<beta>)) (\<alpha> \<^bold>\<Leftarrow> \<beta>) \<in>: funspace (D \<beta>) (D \<alpha>)"
proof -
  have "wff (\<alpha> \<^bold>\<Leftarrow> \<beta>) (Cst (Par f (\<alpha> \<^bold>\<Leftarrow> \<beta>)) (\<alpha> \<^bold>\<Leftarrow> \<beta>))"
    by auto
  then have "val D I \<phi> (Cst (Par f (\<alpha> \<^bold>\<Leftarrow> \<beta>)) (\<alpha> \<^bold>\<Leftarrow> \<beta>)) \<in>: D (\<alpha> \<^bold>\<Leftarrow> \<beta>)"
    using assms unfolding general_model_def by (auto elim: wf_interp.cases)
  then show "I (Par f (\<alpha> \<^bold>\<Leftarrow> \<beta>)) (\<alpha> \<^bold>\<Leftarrow> \<beta>) \<in>: funspace (D \<beta>) (D \<alpha>)"
    using assms subs_def unfolding general_model_def wf_interp.simps wf_frame_def by auto
qed

(* Corresponds to Andrews' theorem 5402 a for rule R *)
theorem theorem_5402_a_rule_R:
  fixes A :: \<open>'cst_sym trm\<close>
...
proof (rule allI, rule allI, rule impI)
  fix D :: "type_sym \<Rightarrow> 's" and I :: "('cst_sym, 's) denotation"
  ...
  have r: "replacement A B C C'"
    using assms(3) using rule_R.cases by (metis LHS_def2 RHS_def2)
  ...
  next
    ...
    show ?case
    proof (rule, rule)
      fix \<phi> :: \<open>'s asg\<close>
      assume asg: "asg_into_frame \<phi> D"
      have \<alpha>': "\<alpha>' = \<beta> \<^bold>\<Leftarrow> \<beta>'"
        using replace_App_left.prems(4) by (metis \<alpha>'_def \<beta>'_def wff_App_type_of)
      from asg have "wff \<alpha>' C"
        using replace_App_left by (metis \<alpha>' \<beta>'_def type_of wff_App')
  ...
  next
    ...
    proof (rule, rule)
      ...
      assume asg: "asg_into_frame \<phi> D"
      have \<alpha>': "\<alpha>' = \<beta> \<^bold>\<Leftarrow> \<beta>'"
        using replace_App_right.prems(4) by (metis \<alpha>'_def \<beta>'_def type_of wff_App')
  ...
  next
    ...
    proof (rule, rule)
      ...
      then have val_C_eql_val_D':
        ...
        using asg replace_App_right replace_Abs by (auto simp: asg_into_frame_fun_upd)
      ...
      proof (rule; rule)
        ...
        have "val D I (\<phi>((x, \<alpha>') := z)) C \<in>: D (type_of C)"
          using DI asg a2 replace_Abs.prems(4) unfolding general_model_def by (auto simp: asg_into_frame_fun_upd)
        moreover
      ...
      have "wff (type_of C) D'"
        using replacement_preserves_type replace_Abs.hyps replace_Abs.prems(2) 
          replace_Abs.prems(3) replace_Abs.prems(4) wff_Abs_type_of unfolding general_model_def by meson
      then have same_type: 
...
theorem Fun_Tv_Tv_frame_subs_funspace:
  ...
  by (metis assms(1) general_model_def wf_frame_def wf_interp.simps)
...
proof (cases "(\<phi> (''g'',oo)) \<cdot> true = true \<and> (\<phi> (''g'',oo)) \<cdot> false = true")
  ...
  then have val: "val D I \<phi> ((g\<^sub>o\<^sub>o \<^bold>\<cdot> T) \<^bold>\<and> (g\<^sub>o\<^sub>o \<^bold>\<cdot> F)) = true"
    using assms by (auto simp add: lemma_5401_e_variant_2 boolean_eq_true lemma_5401_c[OF assms(1,2)] lemma_5401_d[OF assms(1,2)])
  ...
  proof (rule; rule; rule)
    ...
    then have "\<psi> (''x'', Tv) = true \<or> \<psi> (''x'', Tv) = false"
      using asg_into_interp_is_true_or_false assms by meson
  ...
  then have  "val D I \<phi> (\<^bold>[\<^bold>\<forall>''x'':Tv. (g\<^sub>o\<^sub>o \<^bold>\<cdot> x\<^sub>o)\<^bold>]) = true"
    using assms by (auto simp: lemma_5401_g)
  then show ?thesis
...
next
  ...
  then have 0: "\<phi> (''g'', oo) \<in>: funspace (D Tv) (D Tv)"
    using assms(1) assms(2) fun_sym_asg_to_funspace by meson
  ...
  proof
    ...
    have "\<phi> (''g'', oo) \<cdot> true \<in>: boolset"
      using assms(1,2) \<open>\<phi> (''g'', Tv \<^bold>\<Leftarrow> Tv) \<in>: D (Tv \<^bold>\<Leftarrow> Tv)\<close>
      by (meson Fun_Tv_Tv_frame_subs_funspace apply_in_rng assms(2) mem_boolset mem_pow subs_def)
  ...
  have "boolean (satisfies D I \<phi> (g\<^sub>o\<^sub>o \<^bold>\<cdot> T)
    ...
    using False assms(1,2) by (auto simp: lemma_5401_c lemma_5401_d boolean_def)
  ...
  then have 2: "val D I \<phi> (\<^bold>[\<^bold>\<forall>''x'':Tv. (g\<^sub>o\<^sub>o \<^bold>\<cdot> x\<^sub>o)\<^bold>]) = false"
    using assms boolean_def by (auto simp: lemma_5401_g_variant_1)
  then show ?thesis
...
proof (cases "\<phi>(''x'',\<alpha>) = \<phi>(''y'',\<alpha>)")
  ...
  show ?thesis
    ...
    using assms by (auto simp: lemma_5401_f_variant_2 lemma_5401_b_variant_1 boolean_def)
next
  case False
  ...
  have 
    "boolean (satisfies D I \<phi> \<^bold>[Var ''x'' \<alpha> \<^bold>=\<alpha>\<^bold>= Var ''y'' \<alpha>\<^bold>] \<longrightarrow>
       satisfies D I \<phi>
         \<^bold>[(Var ''h'' (Tv \<^bold>\<Leftarrow> \<alpha>) \<^bold>\<cdot> Var ''x'' \<alpha>) \<^bold>=Tv\<^bold>= Var ''h'' (Tv \<^bold>\<Leftarrow> \<alpha>) \<^bold>\<cdot> Var ''y'' \<alpha>\<^bold>]) =
       true"
    using boolean_eq_true by (auto simp: lemma_5401_b)
  then
  show ?thesis
    using assms unfolding axiom_2_def by (auto simp: lemma_5401_f_variant_2)
qed                                   
...
proof (cases "\<phi> (''f'',\<alpha> \<^bold>\<Leftarrow> \<beta>) = \<phi> (''g'',\<alpha> \<^bold>\<Leftarrow> \<beta>)")
  ...
  have "satisfies D I \<phi> \<^bold>[Var ''f'' (\<alpha> \<^bold>\<Leftarrow> \<beta>) \<^bold>=\<alpha> \<^bold>\<Leftarrow> \<beta>\<^bold>= Var ''g'' (\<alpha> \<^bold>\<Leftarrow> \<beta>)\<^bold>]"
    using True assms using wff_Var by (auto simp: lemma_5401_b_variant_2)
  ultimately
  show ?thesis
    unfolding axiom_3_def
    using assms by (auto simp: lemma_5401_b_variant_2)
next
  case False
  then have "\<exists>z. z \<in>: D \<beta> \<and> \<phi> (''f'', \<alpha> \<^bold>\<Leftarrow> \<beta>) \<cdot> z \<noteq> \<phi> (''g'', \<alpha> \<^bold>\<Leftarrow> \<beta>) \<cdot> z"
    using funspace_difference_witness[of "\<phi> (''f'', \<alpha> \<^bold>\<Leftarrow> \<beta>)" "D \<beta>" "D \<alpha>" "\<phi> (''g'', \<alpha> \<^bold>\<Leftarrow> \<beta>)"]
      assms(1,2) fun_sym_asg_to_funspace by fast
  then obtain z where
  ...
  have agree: "agree_off_asg \<psi> \<phi> ''x'' \<beta>"
    using \<psi>_def agree_off_asg_def2 by fast
  have asg: "asg_into_interp \<psi> D I"
    using z\<beta> \<psi>_def asg_into_frame_fun_upd assms(2) by fast
  have "val D I \<psi> ((Var ''f'' (\<alpha> \<^bold>\<Leftarrow> \<beta>)) \<^bold>\<cdot> (Var ''x'' \<beta>)) = \<psi> (''f'',\<alpha> \<^bold>\<Leftarrow> \<beta>) \<cdot> \<psi> (''x'', \<beta>)"
    by auto
  moreover
  ...
  have "val D I \<phi> \<^bold>[Var ''f'' (\<alpha> \<^bold>\<Leftarrow> \<beta>) \<^bold>=\<alpha> \<^bold>\<Leftarrow> \<beta>\<^bold>= Var ''g'' (\<alpha> \<^bold>\<Leftarrow> \<beta>)\<^bold>] = false"
    using False assms(1,2) wff_Var by (auto simp: lemma_5401_b_variant_3)
  ultimately show ?thesis
    unfolding axiom_3_def
    using assms(1,2) by (auto simp: lemma_5401_b)
qed
...
proof -
  ...
  have "val D I \<phi> (\<^bold>[\<^bold>\<lambda>x:\<alpha>. (Cst (Par c \<beta>) \<beta>)\<^bold>] \<^bold>\<cdot> A) = val D I ?\<psi> (Cst (Par c \<beta>) \<beta>)"
    using assms by (meson lemma_5401_a wff_Cst wff_Par)
  then have "val D I \<phi> (\<^bold>[\<^bold>\<lambda>x:\<alpha>. Cst (Par c \<beta>) \<beta>\<^bold>] \<^bold>\<cdot> A) = val D I \<phi> (Cst (Par c \<beta>) \<beta>)"
    by auto
  moreover
  have "wff \<beta> (\<^bold>[\<^bold>\<lambda>x:\<alpha>. Cst (Par c \<beta>) \<beta>\<^bold>] \<^bold>\<cdot> A)"
    using assms by auto
  ultimately
  show ?thesis
...
proof -
  ...
  have "val D I \<phi> (\<^bold>[\<^bold>\<lambda>x:\<alpha>. Var x \<alpha>\<^bold>] \<^bold>\<cdot> A) = val D I \<phi> A"
    using assms wff_Var lemma_5401_a by (metis fun_upd_same val.simps(1))
  ultimately
  show ?thesis
...
proof -
  {
    ...
    then have "val D I (\<psi>((y, \<gamma>) := e)) B \<in>: D (type_of B)"
      using asg_into_frame_fun_upd assms(3,4,6,7) \<psi>_eql unfolding general_model_def by (auto simp: asg_into_frame_fun_upd)
    ...
    have
      ...
      using apply_abstract e_in_D asg_into_frame_fun_upd assms(3,4,6,7) unfolding general_model_def by (auto simp: asg_into_frame_fun_upd)
  ...
  have 
    ...
    using asg_into_frame_fun_upd assms(3,4,6,7) \<psi>_eql val_eql_abstract unfolding general_model_def by (auto simp: asg_into_frame_fun_upd)
  then have 
...
proof -
  ...
  have fr: "(y, \<gamma>) \<notin> vars A"
    using assms(3) unfolding axiom_4_4_side_condition_def by blast
  {
    fix z
    ...
    then have "abstract (D \<gamma>) (D (type_of B)) (\<lambda>z. val D I (\<psi>((y, \<gamma>) := z)) B) \<cdot> z = 
      ...
      using apply_abstract_matchable assms(1,2,4,5) type_of asg_into_frame_fun_upd 
        general_model_def z_in_D by (metis \<phi>'_def)
    ...
    proof -
      have valA: "val D I \<phi>' A \<in>: D \<alpha>"
        using \<phi>'_def asg_into_frame_fun_upd z_in_D assms unfolding general_model_def by (auto simp: asg_into_frame_fun_upd)
      have valB: "val D I (\<phi>'((x, \<alpha>) := val D I \<phi>' A)) B \<in>: D (type_of B)" 
        using asg_into_frame_fun_upd z_in_D assms unfolding general_model_def by (simp add: Az \<psi>_def asg_into_frame_fun_upd) 
...
proof -
...

  {
    ...
    have valA: "val D I \<phi> A \<in>: D \<alpha>"
      using val asg wffA assms(1) unfolding general_model_def by blast
    have "\<forall>t cs. val D I \<phi> \<^bold>[\<^bold>\<lambda>cs:t. B\<^bold>] \<in>: D (\<delta> \<^bold>\<Leftarrow> t)"
      using val asg wffB wff_Abs assms(1) unfolding general_model_def by fast
  ...
  then show ?thesis
    unfolding axiom_4_5_def using lemma_5401_b[OF assms(1,2)] assms abstract_eql
    unfolding general_model_def by auto
qed
...
proof -
  have iden_eql: "iden (D Ind) \<cdot> \<phi> (''y'', Ind) = one_elem_fun (\<phi> (''y'', Ind)) (D Ind)"
  proof -
    have "\<phi> (''y'', Ind) \<in>: D Ind"
      using assms by (simp add: asg_into_frame_def)
    moreover
    have "abstract (D Ind) boolset (\<lambda>y. boolean (\<phi> (''y'', Ind) = y)) \<in>: funspace (D Ind) boolset"
  ...
  have "val D I \<phi> (\<^bold>\<iota> \<^bold>\<cdot> (\<^bold>Q Ind \<^bold>\<cdot> y\<^sub>i)) = val D I \<phi> \<^bold>\<iota> \<cdot> val D I \<phi> (\<^bold>Q Ind \<^bold>\<cdot> y\<^sub>i)"
    by auto
  moreover
  have "... = val D I \<phi> y\<^sub>i"
    using assms iden_eql apply_in_rng wff_Q unfolding general_model_def asg_into_frame_def wf_interp.simps
    by simp
  ultimately
  show ?thesis
...
next
  ...
  then have "replacement A B C C'"
    using rule_R.cases unfolding Eql_def by blast
  show ?case
