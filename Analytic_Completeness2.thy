...
inductive CSort :: \<open>('x, 'fm) sort \<Rightarrow> 'fm cprop \<Rightarrow> bool\<close> where
  CSort_Pred [intro!]: \<open>(\<And>S ps Q. S \<in> C \<Longrightarrow> set ps \<subseteq> S \<Longrightarrow> P ps Q \<Longrightarrow> Q C S) \<Longrightarrow> CSort (Pred P) C\<close>
| CSort_Wits [intro!]: \<open>(\<And>S p. S \<in> C \<Longrightarrow> p \<in> S \<Longrightarrow> \<exists>x. set (W p x) \<union> S \<in> C) \<Longrightarrow> CSort (Wits W) C\<close>

inductive_cases CSort_PredE[elim!]: \<open>CSort (Pred P) C\<close>
inductive_cases CSort_WitsE[elim!]: \<open>CSort (Wits W) C\<close>

inductive (in Params_Fm) ACSort :: \<open>('x, 'fm) sort \<Rightarrow> 'fm cprop \<Rightarrow> bool\<close> where
  ACSort_Pred [intro!]: \<open>(\<And>S ps Q. S \<in> C \<Longrightarrow> set ps \<subseteq> S \<Longrightarrow> P ps Q \<Longrightarrow> Q C S) \<Longrightarrow> ACSort (Pred P) C\<close>
| ACSort_Wits [intro!]: \<open>(\<And>S p x. S \<in> C \<Longrightarrow> p \<in> S \<Longrightarrow> x \<notin> params S \<Longrightarrow> set (W p x) \<union> S \<in> C) \<Longrightarrow> ACSort (Wits W) C\<close>
...
definition (in Params_Fm) ACProp :: \<open>('x, 'fm) sort list \<Rightarrow> 'fm cprop \<Rightarrow> bool\<close> where
  \<open>ACProp Ps C \<equiv> \<forall>P \<in> set Ps. ACSort P C\<close>
...
locale Confl = Params_Fm map_fm params_fm
  for
    map_fm :: \<open>('x \<Rightarrow> 'x) \<Rightarrow> 'fm \<Rightarrow> 'fm\<close> and
    params_fm :: \<open>'fm \<Rightarrow> 'x set\<close> +
  fixes confl_class :: \<open>'fm list \<Rightarrow> 'fm list \<Rightarrow> bool\<close> (infix \<open>\<leadsto>\<^sub>\<crossmark>\<close> 50)
...
next
  fix C
  assume conflC: \<open>CSort confl_sort C\<close>
  then show \<open>ACSort confl_sort (mk_alt_consistency C)\<close>
  proof safe
    fix S ps qs q
...
next
  fix C
  assume conflAC: \<open>ACSort confl_sort C\<close> and closedC: \<open>subset_closed C\<close>
  then show \<open>ACSort confl_sort (mk_finite_char C)\<close>
  proof safe
    fix S ps qs q
...
locale Alpha = Params_Fm map_fm params_fm
  for
    map_fm :: \<open>('x \<Rightarrow> 'x) \<Rightarrow> 'fm \<Rightarrow> 'fm\<close> and
    params_fm :: \<open>'fm \<Rightarrow> 'x set\<close> +
  fixes alpha_class :: \<open>'fm list \<Rightarrow> 'fm list \<Rightarrow> bool\<close> (infix \<open>\<leadsto>\<^sub>\<alpha>\<close> 50)
...
proof
  ...
  then show \<open>CSort (Wits delta_fun) (close C)\<close>
  proof safe
    fix S p qs q
    assume \<open>S \<in> close C\<close>
    then obtain S' where S': \<open>S' \<in> C\<close> and \<open>S \<subseteq> S'\<close>
...
next
  fix C
  assume deltaC: \<open>CSort (Wits delta_fun) C\<close>
  then show \<open>ACSort (Wits delta_fun) (mk_alt_consistency C)\<close>
  proof safe
    fix S p qs x
...
next
  fix C
  assume deltaAC: \<open>ACSort (Wits delta_fun) C\<close> and closedC: \<open>subset_closed C\<close>
  then show \<open>ACSort (Wits delta_fun) (mk_finite_char C)\<close>
  proof safe
    fix S p qs x
    assume \<open>S \<in> mk_finite_char C\<close>
    then have finc: \<open>\<forall>S' \<subseteq> S. finite S' \<longrightarrow> S' \<in> C\<close>
...
theorem CProp: \<open>CProp Ps C \<Longrightarrow> ACProp Ps (mk_cprop C)\<close>
  unfolding mk_cprop_def
  by (simp add: CProp_alt CProp_close CProp_fin close_closed mk_alt_consistency_closed)

end

definition my_confl :: \<open>'fm list \<Rightarrow> 'fm list \<Rightarrow> bool\<close> where \<open>my_confl ps qs \<equiv> True\<close>
definition my_gamma :: \<open>'fm list \<Rightarrow> ('fm set \<Rightarrow> 'tm set) \<times> ('tm \<Rightarrow> 'fm list) \<Rightarrow> bool\<close> where \<open>my_gamma ps qs \<equiv> False\<close>
definition my_delta :: \<open>'fm \<Rightarrow> 'x \<Rightarrow> 'fm list\<close> where \<open>my_delta ps x \<equiv> []\<close>
definition my_modal :: \<open>'fm list \<Rightarrow> ('fm set \<Rightarrow> 'fm set) \<times> 'fm list \<Rightarrow> bool\<close> where \<open>my_modal ps qs \<equiv> False\<close>

interpretation c: Confl \<open>\<lambda>_. id\<close> \<open>\<lambda>_. {}\<close> my_confl
proof qed auto

interpretation g: Gamma \<open>\<lambda>_. id\<close> \<open>\<lambda>_. id\<close> \<open>\<lambda>_. {}\<close> my_gamma
proof qed (auto simp: my_gamma_def)

interpretation d: Delta \<open>\<lambda>_. id\<close> \<open>\<lambda>_. {}\<close> my_delta
proof qed (auto simp: my_delta_def)
...
fun (in Params_Fm) witness_sorts :: \<open>('x, 'fm) sort list \<Rightarrow> 'fm \<Rightarrow> 'fm set \<Rightarrow> 'fm set\<close> where
  \<open>witness_sorts [] p S = {}\<close>
| \<open>witness_sorts (Pred _ # Qs) p S = witness_sorts Qs p S\<close>
| \<open>witness_sorts (Wits W # Qs) p S =
  (let
    rest = witness_sorts Qs p S;
    a = SOME x. x \<notin> params (rest \<union> {p} \<union> S)
   in set (W p a) \<union> rest)\<close>

lemma (in Params_Fm) witness_sorts:
  assumes \<open>Wits W \<in> set Qs\<close>
  shows \<open>\<exists>x. set (W p x) \<subseteq> witness_sorts Qs p S\<close>
  using assms
  by (induct Qs p S rule: witness_sorts.induct) (auto simp add: Let_def)

locale Maximal_Consistency = Consistency_Prop map_fm params_fm Ps
  for
    map_fm :: \<open>('x \<Rightarrow> 'x) \<Rightarrow> 'fm \<Rightarrow> 'fm\<close> and
    params_fm :: \<open>'fm \<Rightarrow> 'x set\<close> and
...
definition witness :: \<open>'fm \<Rightarrow> 'fm set \<Rightarrow> 'fm set\<close> where
  \<open>witness \<equiv> witness_sorts Ps\<close>
...
lemma finite_witness_sorts: \<open>finite (witness_sorts Qs p S)\<close>
  unfolding witness_def by (induct Qs p S rule: witness_sorts.induct) (simp_all add: Let_def)

lemma finite_witness: \<open>finite (witness p S)\<close>
  unfolding witness_def using finite_witness_sorts .

lemma finite_witness_sorts_params: \<open>finite (params (witness_sorts Qs p S))\<close>
  using finite_witness_sorts by simp
...
lemma extend_in_C_step:
  assumes \<open>ACProp Ps C\<close> \<open>{n} \<union> extend C S n \<in> C\<close>
...
proof -
  have \<open>set Qs \<subseteq> set Ps \<Longrightarrow> witness_sorts Qs n (extend C S n) \<union> {n} \<union> extend C S n \<in> C\<close> for Qs
  proof (induct Qs)
    case Nil
    then show ?case
  ...
  next
    ...
    let ?rest = \<open>witness_sorts Qs n ?S\<close>

    have Q: \<open>Q \<in> set Ps\<close>
    ...
    proof (cases Q)
      ...
      have \<open>infinite (UNIV - params (?rest \<union> {n} \<union> ?S))\<close>
        using finite_witness_sorts_params inf by (metis UN_Un Un_assoc infinite_Diff_fin_Un)
      ...
      moreover have \<open>witness_sorts (Q # Qs) n ?S = set (W n a) \<union> ?rest\<close>
...
lemma extend_in_C:
  assumes \<open>ACProp Ps C\<close> \<open>finite_char C\<close> \<open>S \<in> C\<close> \<open>r \<le>o |- params S|\<close> \<open>n \<in> Field r\<close>
...
next
 ...
 then have \<open>i \<in> Field r\<close>
  ...
  then have \<open>infinite (- params ({i} \<union> extend C S i))\<close>
    ...
    by (metis UN_Un c.finite_params_fm finite_UN_I finite_insert finite_params_fm infinite_diff_finite)
  then show ?case
...
lemma Extend_in_C:
  assumes \<open>ACProp Ps C\<close> \<open>finite_char C\<close> \<open>S \<in> C\<close> \<open>r \<le>o |- params S|\<close>
...
theorem mk_mcs_witnessed:
  assumes \<open>CProp Ps C\<close> \<open>S \<in> C\<close> \<open>r \<le>o |- params S|\<close>
...
locale Maximal_Consistency_UNIV = Consistency_Prop map_fm params_fm Ps
  for
    map_fm :: \<open>('x \<Rightarrow> 'x) \<Rightarrow> 'fm \<Rightarrow> 'fm\<close> and
    params_fm :: \<open>'fm \<Rightarrow> 'x set\<close> and
    Ps :: \<open>('x, 'fm) sort list\<close> +
...
TODO: You sort of need to know what the Pred Q does to form a generic Hintikka set
  - When it mandates membership in C, you know that you have membership in H
  - But you don't know that it mandates membership in C

The below is an ad-hoc resolution using the pre-defined locales directly.
*)

locale Hintikka =
  Confl map_fm params_fm confl_class +
  Alpha map_fm params_fm alpha_class +
  Beta map_fm params_fm beta_class +
  Gamma map_tm map_fm params_fm gamma_class +
  Delta map_fm params_fm delta_class
  (* TODO: Modal *)
  for
    map_tm :: \<open>('x \<Rightarrow> 'x) \<Rightarrow> 'tm \<Rightarrow> 'tm\<close> and
    map_fm :: \<open>('x \<Rightarrow> 'x) \<Rightarrow> 'fm \<Rightarrow> 'fm\<close> and
    params_fm :: \<open>'fm \<Rightarrow> 'x set\<close> and
    confl_class :: \<open>'fm list \<Rightarrow> 'fm list \<Rightarrow> bool\<close> (infix \<open>\<leadsto>\<^sub>\<crossmark>\<close> 50) and
    alpha_class :: \<open>'fm list \<Rightarrow> 'fm list \<Rightarrow> bool\<close> (infix \<open>\<leadsto>\<^sub>\<alpha>\<close> 50) and
    beta_class :: \<open>'fm list \<Rightarrow> 'fm list \<Rightarrow> bool\<close> (infix \<open>\<leadsto>\<^sub>\<beta>\<close> 50) and
    gamma_class :: \<open>'fm list \<Rightarrow> ('fm set \<Rightarrow> 'tm set) \<times> ('tm \<Rightarrow> 'fm list) \<Rightarrow> bool\<close> (infix \<open>\<leadsto>\<^sub>\<gamma>\<close> 50) and
    delta_class :: \<open>'fm \<Rightarrow> ('x \<Rightarrow> 'fm list) \<Rightarrow> bool\<close> (infix \<open>\<leadsto>\<^sub>\<delta>\<close> 50)
 +
  fixes H :: \<open>'fm set\<close>
  assumes
    confl: \<open>\<And>ps qs. set ps \<subseteq> H \<Longrightarrow> ps \<leadsto>\<^sub>\<crossmark> qs \<Longrightarrow> \<forall>q \<in> set qs. q \<notin> H\<close> and
    alpha: \<open>\<And>ps qs. set ps \<subseteq> H \<Longrightarrow> ps \<leadsto>\<^sub>\<alpha> qs \<Longrightarrow> \<forall>q \<in> set qs. q \<in> H\<close> and
    beta: \<open>\<And>ps qs. set ps \<subseteq> H \<Longrightarrow> ps \<leadsto>\<^sub>\<beta> qs \<Longrightarrow> \<exists>q \<in> set qs. q \<in> H\<close> and
    gamma: \<open>\<And>ps F qs. set ps \<subseteq> H \<Longrightarrow> ps \<leadsto>\<^sub>\<gamma> (F, qs) \<Longrightarrow> \<forall>t \<in> P H. set (qs t) \<subseteq> H\<close> and
    delta: \<open>\<And>p qs. p \<in> H \<Longrightarrow> p \<leadsto>\<^sub>\<delta> qs \<Longrightarrow> \<exists>x. set (qs x) \<subseteq> H\<close>
...
(* 
  ...
  I want to say that given Confl etc. then the properties are true
*)
theorem Hintikka_MCS:
  assumes \<open>ACProp Ps C\<close> \<open>S \<in> C\<close> \<open>maximal C S\<close> \<open>witnessed S\<close>
  shows \<open>Hintikka map_tm map_fm params_fm confl_class alpha_class beta_class gamma_class delta_class S\<close>
proof
  fix ps qs
  assume *: \<open>set ps \<subseteq> S\<close> \<open>confl_class ps qs\<close>
  then show \<open>\<forall>q \<in> set qs. q \<notin> S\<close>
    using \<open>S \<in> C\<close> \<open>ACProp Ps C\<close> unfolding ACProp_def ACSort_def
next
  fix ps qs
  assume *: \<open>set ps \<subseteq> S\<close> \<open>ps \<leadsto> Alpha qs\<close>
  then have \<open>set qs \<union> S \<in> C\<close>
    using * \<open>S \<in> C\<close> \<open>alt_consistency C\<close> unfolding alt_consistency_def by fasts
...
locale Confl = Map_Fm map_fm
  for map_fm :: \<open>('x \<Rightarrow> 'x) \<Rightarrow> 'fm \<Rightarrow> 'fm\<close> +
  fixes confl_class :: \<open>'fm list \<Rightarrow> 'fm list \<Rightarrow> bool\<close> (infix \<open>\<leadsto>\<^sub>\<crossmark>\<close> 50)
  assumes confl_map: \<open>respects_map (\<leadsto>\<^sub>\<crossmark>)\<close>

locale Alpha = Map_Fm map_fm
  for map_fm :: \<open>('x \<Rightarrow> 'x) \<Rightarrow> 'fm \<Rightarrow> 'fm\<close> +
  fixes alpha_class :: \<open>'fm list \<Rightarrow> 'fm list \<Rightarrow> bool\<close> (infix \<open>\<leadsto>\<^sub>\<alpha>\<close> 50)
  assumes alpha_map: \<open>respects_map (\<leadsto>\<^sub>\<alpha>)\<close>

locale Beta = Map_Fm map_fm
  for map_fm :: \<open>('x \<Rightarrow> 'x) \<Rightarrow> 'fm \<Rightarrow> 'fm\<close> +
  fixes beta_class :: \<open>'fm list \<Rightarrow> 'fm list \<Rightarrow> bool\<close> (infix \<open>\<leadsto>\<^sub>\<beta>\<close> 50)
  assumes beta_map: \<open>respects_map (\<leadsto>\<^sub>\<beta>)\<close>

locale Gamma = Map_Tm map_tm + Map_Fm map_fm
  for
    map_tm :: \<open>('x \<Rightarrow> 'x) \<Rightarrow> 'tm \<Rightarrow> 'tm\<close> and
    map_fm :: \<open>('x \<Rightarrow> 'x) \<Rightarrow> 'fm \<Rightarrow> 'fm\<close> +
  fixes gamma_class :: \<open>'fm list \<Rightarrow> ('fm set \<Rightarrow> 'tm set) \<times> ('tm \<Rightarrow> 'fm list) \<Rightarrow> bool\<close> (infix \<open>\<leadsto>\<^sub>\<gamma>\<close> 50)
  (* TODO: make this nice? *)
  assumes gamma_map: \<open>\<And>ps F qs f. ps \<leadsto>\<^sub>\<gamma> (F, qs) \<Longrightarrow> \<exists>G rs. map (map_fm f) ps \<leadsto>\<^sub>\<gamma> (G, rs) \<and>
      (\<forall>S. map_tm f ` F S \<subseteq> G (map_fm f ` S)) \<and>
      (\<forall>t. map (map_fm f) (qs t) = rs (map_tm f t))\<close>
    and gamma_mono: \<open>\<And>ps P qs S S'. ps \<leadsto>\<^sub>\<gamma> (P, qs) \<Longrightarrow> S \<subseteq> S' \<Longrightarrow> P S \<subseteq> P S'\<close>
    and gamma_fin: \<open>\<And>ps P qs t A. ps \<leadsto>\<^sub>\<gamma> (P, qs) \<Longrightarrow> t \<in> P A \<Longrightarrow> \<exists>B \<subseteq> A. finite B \<and> t \<in> P B\<close>


locale Delta = Map_Fm map_fm
  for map_fm :: \<open>('x \<Rightarrow> 'x) \<Rightarrow> 'fm \<Rightarrow> 'fm\<close> +
  fixes delta_class :: \<open>'fm list \<Rightarrow> ('x \<Rightarrow> 'fm list) \<Rightarrow> bool\<close> (infix \<open>\<leadsto>\<^sub>\<delta>\<close> 50)
  (* TODO: make this nice? *)
  assumes delta_map: \<open>\<And>ps qs f. ps \<leadsto>\<^sub>\<delta> qs \<Longrightarrow> \<exists>rs. map (map_fm f) ps \<leadsto>\<^sub>\<delta> rs \<and>
      (\<forall>x. map (map_fm f) (qs x) = rs (f x))\<close>
    and delta_singl: \<open>\<And>ps qs. ps \<leadsto>\<^sub>\<delta> qs \<Longrightarrow> \<exists>p. ps = [p]\<close>
    and delta_uniq: \<open>\<And>ps qs rs. ps \<leadsto>\<^sub>\<delta> qs \<Longrightarrow> ps \<leadsto>\<^sub>\<delta> rs \<Longrightarrow> qs = rs\<close>

locale Consistency =
  Params_Fm map_fm params_fm + 
  Confl map_fm confl_class +
  Alpha map_fm alpha_class +
  Beta map_fm beta_class +
  Gamma map_tm map_fm gamma_class +
  Delta map_fm delta_class
  for
    map_tm :: \<open>('x \<Rightarrow> 'x) \<Rightarrow> 'tm \<Rightarrow> 'tm\<close> and
    map_fm :: \<open>('x \<Rightarrow> 'x) \<Rightarrow> 'fm \<Rightarrow> 'fm\<close> and
    params_fm :: \<open>'fm \<Rightarrow> 'x set\<close> and
    ...
    beta_class :: \<open>'fm list \<Rightarrow> 'fm list \<Rightarrow> bool\<close> (infix \<open>\<leadsto>\<^sub>\<beta>\<close> 50) and
    gamma_class :: \<open>'fm list \<Rightarrow> ('fm set \<Rightarrow> 'tm set) \<times> ('tm \<Rightarrow> 'fm list) \<Rightarrow> bool\<close> (infix \<open>\<leadsto>\<^sub>\<gamma>\<close> 50) and
    delta_class :: \<open>'fm list \<Rightarrow> ('x \<Rightarrow> 'fm list) \<Rightarrow> bool\<close> (infix \<open>\<leadsto>\<^sub>\<delta>\<close> 50) +
  fixes C :: \<open>'fm cprop\<close>
  assumes
    conflC: \<open>\<And>S ps qs q. S \<in> C \<Longrightarrow> set ps \<subseteq> S \<Longrightarrow> ps \<leadsto>\<^sub>\<crossmark> qs \<Longrightarrow> q \<in> set qs \<Longrightarrow> q \<notin> S\<close> and
    alphaC: \<open>\<And>S ps qs. S \<in> C \<Longrightarrow> set ps \<subseteq> S \<Longrightarrow> ps \<leadsto>\<^sub>\<alpha> qs \<Longrightarrow> set qs \<union> S \<in> C\<close> and
    betaC: \<open>\<And>S ps qs. S \<in> C \<Longrightarrow> set ps \<subseteq> S \<Longrightarrow> ps \<leadsto>\<^sub>\<beta> qs \<Longrightarrow> \<exists>q \<in> set qs. {q} \<union> S \<in> C\<close> and
    gammaC: \<open>\<And>S ps F qs t. S \<in> C \<Longrightarrow> set ps \<subseteq> S \<Longrightarrow> ps \<leadsto>\<^sub>\<gamma> (F, qs) \<Longrightarrow> t \<in> F S \<Longrightarrow> set (qs t) \<union> S \<in> C\<close> and
    deltaC: \<open>\<And>S ps qs. S \<in> C \<Longrightarrow> set ps \<subseteq> S \<Longrightarrow> ps \<leadsto>\<^sub>\<delta> qs \<Longrightarrow> \<exists>x. set (qs x) \<union> S \<in> C\<close>

locale Close_Consistency = Consistency

sublocale Close_Consistency \<subseteq> Consistency map_tm map_fm params_fm confl_class alpha_class beta_class gamma_class delta_class \<open>close C\<close>
proof
  fix S ps qs
  assume \<open>S \<in> close C\<close>
  then obtain S' where S': \<open>S' \<in> C\<close> and \<open>S \<subseteq> S'\<close>
  ...
  {
    fix q
    assume **: \<open>ps \<leadsto>\<^sub>\<crossmark> qs\<close> \<open>q \<in> set qs\<close>
    then have \<open>\<forall>q \<in> set qs. q \<notin> S'\<close>
      using conflC S' * by blast
    then have \<open>\<forall>q \<in> set qs. q \<notin> S\<close>
      using \<open>S \<subseteq> S'\<close> by blast
...

  {
    fix t F
    assume \<open>ps \<leadsto>\<^sub>\<gamma> (F, qs)\<close> \<open>t \<in> F S\<close>
    moreover from this have \<open>t \<in> F S'\<close>
      using gamma_mono \<open>S \<subseteq> S'\<close> by blast
    ultimately have \<open>set (qs t) \<union> S' \<in> C\<close>
      using gammaC S' * by blast
    then have \<open>set (qs t) \<union> S \<in> close C\<close>
  ...
  {
    assume \<open>ps \<leadsto>\<^sub>\<delta> qs\<close>
    then have \<open>\<exists>x. set (qs x) \<union> S' \<in> C\<close>
      using deltaC S' * by blast
...
locale Alt_Consistency =
  ...
  fixes C :: \<open>'fm cprop\<close>
  assumes
    conflAC: \<open>\<And>S ps qs q. S \<in> C \<Longrightarrow> set ps \<subseteq> S \<Longrightarrow> ps \<leadsto>\<^sub>\<crossmark> qs \<Longrightarrow> q \<in> set qs \<Longrightarrow> q \<notin> S\<close> and
    alphaAC: \<open>\<And>S ps qs. S \<in> C \<Longrightarrow> set ps \<subseteq> S \<Longrightarrow> ps \<leadsto>\<^sub>\<alpha> qs \<Longrightarrow> set qs \<union> S \<in> C\<close> and
    betaAC: \<open>\<And>S ps qs. S \<in> C \<Longrightarrow> set ps \<subseteq> S \<Longrightarrow> ps \<leadsto>\<^sub>\<beta> qs \<Longrightarrow> \<exists>q \<in> set qs. {q} \<union> S \<in> C\<close> and
    gammaAC: \<open>\<And>S ps F qs t. S \<in> C \<Longrightarrow> set ps \<subseteq> S \<Longrightarrow> ps \<leadsto>\<^sub>\<gamma> (F, qs) \<Longrightarrow> t \<in> F S \<Longrightarrow> set (qs t) \<union> S \<in> C\<close> and
    deltaAC: \<open>\<And>S ps qs x. S \<in> C \<Longrightarrow> set ps \<subseteq> S \<Longrightarrow> ps \<leadsto>\<^sub>\<delta> qs \<Longrightarrow> x \<notin> params S \<Longrightarrow> set (qs x) \<union> S \<in> C\<close>

sublocale Consistency \<subseteq>
  Alt_Consistency map_tm map_fm params_fm confl_class alpha_class beta_class gamma_class delta_class \<open>mk_alt_consistency C\<close>
proof
  ...
  {
    fix x
    have delta_map': \<open>\<And>ps qs f. ps \<leadsto>\<^sub>\<delta> qs \<Longrightarrow>
        \<exists>rs. map (map_fm f) ps \<leadsto>\<^sub>\<delta> rs \<and> (\<forall>x. map (map_fm f) (qs x) = rs (f x))\<close>
      using delta_map by blast

    assume **: \<open>ps \<leadsto>\<^sub>\<delta> qs\<close> and x: \<open>x \<notin> params S\<close>
    then obtain rs where rs: \<open>map (map_fm f) ps \<leadsto>\<^sub>\<delta> rs\<close> \<open>\<forall>x. map (map_fm f) (qs x) = rs (f x)\<close>
      using delta_map' by meson
    ...
    then have \<open>map (map_fm ?g) ps \<leadsto>\<^sub>\<delta> rs\<close> \<open>\<forall>x. map (map_fm ?g) (qs x) = rs (?g x)\<close>
      using rs delta_map' delta_uniq ** by metis+
...
locale Finite_Alt_Consistency = Alt_Consistency +
  assumes closedC: \<open>subset_closed C\<close>

sublocale Finite_Alt_Consistency \<subseteq>
  Alt_Consistency map_tm map_fm params_fm confl_class alpha_class beta_class gamma_class delta_class \<open>mk_finite_char C\<close>
proof
  fix S ps qs
  assume \<open>S \<in> mk_finite_char C\<close>
  then have finc: \<open>\<forall>S' \<subseteq> S. finite S' \<longrightarrow> S' \<in> C\<close>
...

  {
    fix q
    assume **: \<open>ps \<leadsto>\<^sub>\<crossmark> qs\<close> \<open>q \<in> set qs\<close>
    then have \<open>{q} \<union> set ps \<in> C\<close> if \<open>q \<in> S\<close>
      using that \<open>set ps \<subseteq> S\<close> finc by simp
    then show \<open>q \<notin> S\<close>
      using ** conflAC by fast
  }
  ...
  {
    fix x
    assume **: \<open>ps \<leadsto>\<^sub>\<delta> qs\<close> and \<open>x \<notin> params S\<close>
...
(* TODO: move these? *)
definition (in Map_Fm) mk_cprop :: \<open>'fm cprop \<Rightarrow> 'fm cprop\<close> where
  \<open>mk_cprop C \<equiv> mk_finite_char (mk_alt_consistency (close C))\<close>

lemma (in Map_Fm) mk_cprop_subset_closed: \<open>subset_closed (mk_cprop C)\<close>
  unfolding mk_cprop_def using finite_char finite_char_closed by blast

lemma (in Map_Fm) mk_cprop_finite_char: \<open>finite_char (mk_cprop C)\<close>
  unfolding mk_cprop_def using finite_char by blast

sublocale Close_Consistency \<subseteq> Finite_Alt_Consistency map_tm map_fm params_fm confl_class alpha_class beta_class gamma_class delta_class \<open>mk_alt_consistency (close C)\<close>
proof
  show \<open>subset_closed (mk_alt_consistency (close C))\<close>
    by (simp add: close_closed mk_alt_consistency_closed)
qed

locale Make_CProp = Consistency

sublocale Make_CProp \<subseteq> Close_Consistency
proof qed

sublocale Make_CProp \<subseteq> Alt_Consistency map_tm map_fm params_fm confl_class alpha_class beta_class gamma_class delta_class \<open>mk_alt_consistency (close C)\<close>
  unfolding mk_cprop_def proof qed

sublocale Make_CProp \<subseteq> Alt_Consistency map_tm map_fm params_fm confl_class alpha_class beta_class gamma_class delta_class \<open>mk_cprop C\<close>
  unfolding mk_cprop_def proof qed

lemma (in Make_CProp) mk_cprop_in: \<open>S \<in> C \<Longrightarrow> S \<in> mk_cprop C\<close>
  using close_subset closedC finite_char_subset mk_alt_consistency_subset mk_cprop_def by fastforce

locale Maximal_Consistency =
  (* TODO: some duplication with Consistency? *)
  Params_Fm map_fm params_fm + 
  Confl map_fm confl_class +
  Alpha map_fm alpha_class +
  Beta map_fm beta_class +
...
definition witness :: \<open>'fm \<Rightarrow> 'fm set \<Rightarrow> 'fm set\<close> where
  \<open>witness p S \<equiv> if \<exists>qs. [p] \<leadsto>\<^sub>\<delta> qs
    then let a = SOME x. x \<notin> params ({p} \<union> S) in set ((THE qs. [p] \<leadsto>\<^sub>\<delta> qs) a)
    else {}\<close>

lemma witness:
  assumes \<open>[p] \<leadsto>\<^sub>\<delta> qs\<close>
  shows \<open>witness p S = set (qs (SOME x. x \<notin> params ({p} \<union> S)))\<close>
  unfolding witness_def using assms delta_uniq by (metis the_equality)

lemma no_witness:
  assumes \<open>\<nexists>qs. [p] \<leadsto>\<^sub>\<delta> qs\<close>
...
lemma extend_in_C_step:
  ...
  shows \<open>extend C S (succ r n) \<in> C\<close>
proof (cases \<open>\<exists>qs. [n] \<leadsto>\<^sub>\<delta> qs\<close>)
  case True
  then obtain qs where Delta: \<open>[n] \<leadsto>\<^sub>\<delta> qs\<close>
    by blast
